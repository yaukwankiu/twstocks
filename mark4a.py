...
class stock:
    ...
    def plot(self, display=False, imagePath="", block=False): #special setting for 801
        plt.close()
...
def main2(#toWatch="fixed",
    ...
    while True:
        ...
        while not isTradingHour():
            if toWatch =='random':
                watchRandom(stocks=stocks, imagePath='default')
                ...
                watchRandom(stocks=stocks, timeSleep=timeSleep, imagePath='default')
                watchRandom(stocks=stocks, timeSleep=timeSleep, imagePath=chartsFolder+'temp.png')
